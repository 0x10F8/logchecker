...
/**
 * Factory for generating {@link Rule} implementations parsed from JSON
 * configuration files.
 */
public final class RuleFactory {
	 ...
	 *
	 * @param ruleJsonFiles A list of files containing the JSON configurations for
	 *                      the rules
	 *
	...
	public static List<Rule> loadRulesFromFiles(final List<File> ruleJsonFiles) throws IOException {
		...
		for (final File ruleFile : ruleJsonFiles) {
			final Rule fromFile = gson.fromJson(new String(Files.readAllBytes(Paths.get(ruleFile.toURI()))),
					Rule.class);
